    ...
    <div style={{ maxWidth: 860, margin: '0 auto' }}>
      ...
      <Section id="produccion" title="6. Producción" badge="Nuevo" badgeColor="#0EA5E9" open={open === 'produccion'} onToggle={() => toggle('produccion')}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <Step n={6} title="Revisá sin salir de la herramienta" desc="Desde que una pieza se entrega, la tarjeta muestra una previa del enlace: hacé clic para verla en grande. Sale de un archivo de Drive compartido como 'cualquiera con el enlace'; si el enlace no permite previa —Figma, o un archivo restringido— la tarjeta lo dice en vez de mostrar una imagen rota." />
          <Step n={7} title="Comentá lo que haga falta" desc="En la orden de trabajo hay un campo de comentarios para lo que hay que decir sobre la pieza y no es un motivo de devolución. Queda en el historial, con autor y fecha, y no cambia el estado de la pieza." />
          <Step n={8} title="Exportá lo que quedó Lista" desc="La columna Lista tiene un botón para bajar un Excel con dos hojas: 'Piezas' —una fila por pieza, con su enlace, formato y fecha de aprobación— y 'Copy' —una fila por slot con el texto exacto, listo para pegar en el programador. El brief de producción viaja marcado como tal para que nadie lo publique por error." />
          <Step n={9} title="Mis piezas" desc="Cambiá la vista con el selector de arriba a la derecha. El tablero es por marca, pero Mis piezas cruza todas las marcas del workspace y muestra solo lo tuyo." />
        </div>
