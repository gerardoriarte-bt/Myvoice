...
| H1.B | **Motor serio** — telemetría de costo, cuota real, resiliencia, evals | ✅ **B0–B3 desplegados** (2026-08-28) · B4 pendiente | Los cuatro criterios verificados. En producción: 38 generaciones con workspace, 30 con costo (USD 4,28 materializados), 28 piezas con slot. `QUOTA_ENFORCE=false`: mide y avisa sin bloquear. **La medición de consumo se reformula en [E6](./plan-e6-registro-consumo.md)**: la calibración de la cuota y el `QUOTA_ENFORCE=true` esperan a ese plan. 2026-09-16: Refinar pasó a correr por canal y en lotes, porque moría por timeout (PR #7). [despliegue](./despliegue-h1.md) |
...
| # | Iniciativa | Detalle |
|---|---|---|
| H2.A + H2.B | **Del copy aprobado a la pieza verificada** · EN DISEÑO | Una sola fase, no dos iniciativas: el proceso tiene seis etapas y las dos del medio no existen — asignar la pieza al equipo de diseño, y auditar con IA la pieza que ese equipo sube (contra el copy aprobado y contra el ADN). Arranca por el diseño en el `.pen`. **Fase 0 cerrada** el 2026-09-22: las seis decisiones y los cinco estados límite, dibujados. **Nivel 2 escrito**, D7 confirmada y **fase 2 en construcción**: modelo, máquina de estados, API y tablero, con `verify:isolation` en 44 chequeos. **No entra en el despliegue del H1.** [plan de fase](./plan-h2-produccion-auditoria.md) |
| H2.E | **La segunda ronda: el cliente aprueba la pieza** · fases 1 y 2 ✅ **DESPLEGADAS** el 2026-09-26, falta la 3 | Hoy el cliente aprueba un texto y **nunca ve el arte final**: `ReviewSessionItem` apunta solo a `SavedVariation`. Se agrega una segunda ronda sobre la pieza terminada —después del visto bueno interno, opcional por campaña— con el feedback desglosado en copy y diseño. Ese desglose es el enrutador: lo de diseño va al diseñador, lo de copy vuelve al motor **como propuesta**, porque una clasificación equivocada le enseña basura a la IA. No reemplaza la ronda 1: mover la aprobación al final pondría la decisión del cliente después del gasto de diseño. [plan](./plan-h2e-aprobacion-de-pieza.md) |
...
| H2.D | **Aprendizaje de marca evolutivo · y recuperación** | Hoy los ejemplos de voz son los **primeros seis**, sin criterio de relevancia (`promptBuilder.ts:58`, `slice(0, 6)`). Con 28 piezas da igual; con 500, elegir los seis más parecidos a lo que se va a escribir sí cambia el resultado: ahí sí paga recuperación por embeddings. Lo mismo con el manual de marca, del que hoy se usan los primeros 18.000 caracteres y **el resto se descarta** (`brandExtractionService.ts:14`): buscar el fragmento pertinente es recuperación, no extracción. Y con la auditoría de la fase H2, que gana mucho si compara contra el manual completo y no contra la lista corta de prohibiciones. Sumar fingerprint por canal + ranking de ángulos aprobados (`GenerationLog.outputJson` ya tiene la materia prima). **Las tres necesitan que el PDF original siga existiendo**, que es el argumento para no descartarlo tras extraer. |
...
| H3.D | **Funciones del equipo y notificaciones** · ✅ **DESPLEGADA** el 2026-09-23 | Quién escribe, quién diseña y quién aprueba —un eje aparte del permiso, que sugiere y no restringe— y que a cada uno le llegue lo que le toca. Dos eventos y nada más: asignación y entrega. De paso arregla que hoy la única notificación que existe **va a una casilla fija del entorno**, no a la persona. [plan](./plan-h3d-funciones-notificaciones.md) |
...
|---|---|---|---|
| E1 | **Migrar uploads a S3** | ✅ **Hecho** (2026-08-28) | Desplegado: driver S3 activo, `/uploads` devuelve 404 y el bucket rechaza toda lectura sin firma. Cerró de paso el agujero por el que **cualquiera con la URL descargaba la guía de marca de cualquier empresa** —el único lugar donde el aislamiento multi-tenant no llegaba— y dos bugs que dejaban archivos huérfanos: borrar el manual no borraba el archivo, y desde el lote H1 ni siquiera limpiaba la base. Había 53 MB de PDF sin dueño en producción. [plan](./plan-e1-almacenamiento.md) |
...
| E6 | **Registro de consumo por evento** | Media · **se hace al final** | Reemplaza `UsagePeriod` (un contador mutable por marca y mes) por un log append-only con un evento por intento de llamada. Hoy 4 de las 9 llamadas a IA no se miden (refinar, extracción de ADN, fingerprint, prueba de key) y ningún intento fallido queda registrado. Va al final a propósito, para diseñar la tabla una vez con todos los eventos a la vista: cada funcionalidad nueva que consuma algo agrega su fila al inventario del plan. Alimenta el log de consumo de la agencia, que mide todas las herramientas del ecosistema. [plan](./plan-e6-registro-consumo.md) |
