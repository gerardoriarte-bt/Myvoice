...
| E2 | **Gate de tipos real + tests + CI** | **Crítica** | **(1) hecho:** `@types/react` instalado, tsconfig raíz sin `server/`, `tsconfig.scripts.json` cubriendo `scripts/` y `prisma/` con `strict`. La gate pasó de certificar en verde ~8.900 líneas que no leía a leerlas de verdad; destapó 3 errores en `App.tsx`, uno un bug de datos preexistente ([oráculo H1](./oraculo-h1.md) F1, F2, F4, F5), todos arreglados. Falta: (2) `strict: true` en el raíz, con su propio presupuesto — destapa mucho más; (3) CI con las tres gates + `verify:resiliencia` + `verify:isolation` contra un Postgres del runner, 1–2 días; (4) tipar `apiRequest<T>` en `services/api.ts`, hoy `any` en todas las respuestas, 2–3 días; (5) tests de `validators.ts` y eval harness. Cero tests sobre ~8.900 líneas de frontend y ~3.100 de backend; no hay `.github/`. |
